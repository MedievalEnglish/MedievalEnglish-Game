function usenote8(){
	...
	if (enemy1 != noone && variable_instance_exists(enemy1, "lock")){//si no existe un enemigo y no existe alguna de las variables declaradas con lock entonces no hacer nada
		...
		else if (villager3.lock == ITEM_NOTE9) {
			scrTxtbox("Tu respuesta no es correcta, no puedes avanzar");	
		}
		else if (villager4.lock == ITEM_NOTE14) {
			scrTxtbox("Tu respuesta no es correcta, no puedes avanzar");	
		}
		else {
